  ...
  return (
    ...
    >
      ...
      <img
        ...
        className="w-[15%] drop-shadow-2xl rounded-xl shadow-2xl avatar-img max-lg:w-3/4"
      />
      <h1 className="text-6xl font-extrabold">
        {language === "SK"
          ? headerIntroData.title.sk
          : headerIntroData.title.en}
      </h1>
      <h2>{headerIntroData.subtitle}</h2>
      <p className="w-1/2 text-center max-lg:hidden">
        {language === "SK"
          ? headerIntroData.description.sk
      ...
      <div className="button-container flex items-center justify-center mr-8 gap-10 mb-12 max-lg:flex-col max-lg:items-center">
        {headerIntroData.buttons.map((button, index) => (
          <Button
            key={index}
            label={language === "SK" ? button.label.sk : button.label.en}
            iconSVG={button.icon}
